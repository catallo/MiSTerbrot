...
- Current fitted build resource usage (post-A2 + A3 + A7 + cy=0 fix + Attract Mode + Color Depth + Color Cycling submenu + Palette Crossfade + 3-eval restructure + P2 periodicity):
  - ALMs at `89%` — crossfade peaked at 92% with 4 parallel palette evaluators; the staged three-evaluator restructure (2026-07-08) recovered ~4%, and P2 periodicity state re-spent ~1%.  See `output_files/MiSTerbrot.fit.summary` after each compile.
...
- **Timing state (2026-07-08 night, seed 5, placement effort 4.0, shipping build with P2):** `clk_sys +1.84` ns, HDMI PLL ~0, **`clk_iter -0.116` ns worst / `-0.14` TNS (two endpoints) at the 85 °C slow-corner model** — a 9-seed sweep on the P2 netlist found no closing placement (best rolls: −0.116/−0.42/−0.49); the deficit is 1.2% of the period, comfortably inside room-temperature corner margin, and P2 is OSD-disableable (`O[51]`) as a belt-and-braces.  The pre-P2 build (commit `12164c8` netlist) **fully closed all clocks** (`clk_iter +0.060`, `clk_sys +2.17`, seed 5) — rebuild that commit if a timing-clean bitstream is ever needed.  What got clk_sys healthy: (1) SDC multicycle constraints encoding the ce_pix video cadence (it had been silently failing at `-5.8` ns since crossfade), (2) the color_mapper three-evaluator restructure (every evaluator input registered), (3) the final color capture moved to tick+1 (`ce_d1`), giving the blend cone a genuine 2-cycle budget.  Margins are seed-sensitive — re-sweep after any netlist change (parallel scratchpad builds, see memory).
...
- Project headroom (post-3-eval restructure, 2026-07-08):
  ...
  - `77%` RAM blocks
  - `89%` ALMs — recovered from the 92% crossfade peak via the color_mapper three-evaluator restructure, then ~1% re-spent on P2 periodicity state (24 × 48-bit snapshots + compares).  Next ALM-recovery candidates if Track B / periodicity work needs room: text_overlay (4.9k ALUTs), auto_zoom zoom-compare cone (4.4k ALUTs).
