...
- **Deinterlace selector removed** (2026-07-11, user call): `VGA_F1` is permanently suppressed and `HDMI_BOB_DEINT` never requested — the scaler shows each field as an independent progressive half-picture (the former "Off" mode, now the only behavior; no weave combing, no bob shimmer, half vertical detail at the scaler).  Real 15 kHz displays interlace from the half-line sync cadence, which the video timing carries without F1.  status[58:57] are free again.  Side effect: scaler captures of the interlaced modes are now 240-tall (1×1 per field) — the walkthrough tooling normalizes this.
...
- **640×480i mode** (2026-07-09, Track B): the same interlace scheme at 640 wide — 307,200 px = 2× one BRAM bank, so this mode renders into a **DDR3 framebuffer** through the emu `DDRAM_*` port (`rtl/fb_ddr3.v`: write FIFO draining as byte-enabled single-beat writes + ping-pong BRAM line buffer filled by 4 pipelined 40-beat bursts; scanout never touches DDR3 directly).  `video_timing` emits a per-line `prefetch_req`/`prefetch_row` (logical row, one line ahead) driving the fetch.  The BRAM banks still serve the other three modes bit-identically (pair-TB-proven); write-FIFO entries capture their target bank at push, so bank swaps need no drain gating.  `LED_USER` doubles as a sticky line-fetch-underrun alarm.  fps ≈ ¼ of 320×240.  Silicon-probed bandwidth numbers and the full design in `docs/TRACKB_DESIGN.md`.
...
- Resolution is selected at runtime via the unified OSD selector `status[56:54]` (320×240 / 640×240 / 320×480i / 640×480i / 640×480p / 1920×1080 Gallery); `mode_640`/`interlace_mode`/`mode_480p`/`ddr_fb_mode`/`gallery_mode` derive from it in `fractal_top`.  The J key cycles the six modes; an OSD change releases the key override (most recently used source wins).
- **640×480p mode** (2026-07-10): 525 progressive lines at 31.25 kHz / ~59.5 Hz (V: 480+10+3+32; H structure unchanged at 800 → 25 MHz ce_pix, /4 in the 100 MHz video domain).  Uses the direct dvid output path like the interlaced modes (nothing to scandouble at 31 kHz); scandoubler fx forced off; overlay glyph rows are halved so the text layout matches every other mode; A2 symmetry bounds key on `render_480` (interlace OR 480p — the mirror rows span the full 480-line frame).  Same DDR3 framebuffer and boot-grace policy as 640×480i.  See `docs/480P_DESIGN.md`.  **Boot grace**: the core always starts progressive and switches to a saved 480i setting after ~60 vblanks — the framework's very first mode lock after core load cannot cope with an interlaced signal (PSX never boots interlaced; found the hard way).  Every resolution change toggles `new_vmode` toward hps_io so the ARM re-measures immediately.
...
- `rtl/gallery_fb.v`: Gallery DDR3 index-buffer engine — write FIFO draining as single-byte-enable 64-bit writes (Track-B probe pattern), framework FB config (8bpp indexed, 1920×1080, stride 2048, double-buffer `FB_BASE`), `FB_PAL` pass-through, overflow-drop guard
- `rtl/gallery_ctl.v`: Gallery controller — activation clear (both banks to index 0), live/hidden bank policy, palette fade + tear-free-by-blackness `FB_BASE` flip, dwell qualifier
- `rtl/gallery_palette.v`: Gallery palette sequencer — injects iter=1..255 through the idle display color pipeline each vblank, captures the pipelined RGB (bit-exact vs per-pixel math, golden-model TB), streams `FB_PAL`, applies the fade scale
- `rtl/gallery_pitch.v`: free-running serial multiplier `pitch = step × 2/9` for the square-pixel 1080p grid (≤1.5 ulp)
...
| `O[56:54]`| Resolution (320×240 / 640×240 / 320×480i / 640×480i / 640×480p / 1920×1080) — unified selector.  Both 640×480 modes render into the DDR3 framebuffer (`rtl/fb_ddr3.v`, `docs/TRACKB_DESIGN.md`); Buffer option ignored there (always double).  640×480p is progressive 31.25 kHz (525 lines, direct dvid path, no deinterlacing; `docs/480P_DESIGN.md`).  1920×1080 = Gallery Mode: zoom-less, renders each POI once as 8-bit indices into a framework framebuffer (`MISTER_FB`+`MISTER_FB_PALETTE`, ascal scans DDR3 directly), color cycling via per-frame palette rewrites through the idle display color pipeline (`docs/GALLERY_DESIGN.md`) |
...
| `O[59]`   | Gallery Live Render (On default / Off) — On paints each POI progressively into the displayed buffer; Off renders hidden, then palette-fades to black, flips `FB_BASE` while the screen is uniformly black (tear-free), and fades back in.  Greyed outside gallery (`d0`); the zoom-related Attract rows grey inside it (`D0`, `status_menumask[0]`) |
