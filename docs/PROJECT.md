...
- Resolution is selected at runtime via the unified OSD selector `status[56:54]` (320×240 / 640×240 / 320×480i / 640×480i / 640×480p); `mode_640`/`interlace_mode`/`mode_480p`/`ddr_fb_mode` derive from it in `fractal_top`.  The J key cycles the five modes; an OSD change releases the key override (most recently used source wins).
