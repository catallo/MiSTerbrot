...
initial begin
    status[56:54] = 3'd4;   // Resolution: 640x480p
        ...
        if (((addr - BASE) % BANKW) % 256 == 0) begin
            ...
            begin : rowchk
                ...
                if (have_lrow && lrow != last_lrow) begin
                    // progressive rows advance +1; wraps land on row 0
                    if (!(lrow == last_lrow + 10'd1 || lrow == 10'd0)) begin
                        seq_errs = seq_errs + 1;
                        if (seq_errs < 5)
...
initial begin
    ...
    begin : wait_il
        integer guard;
        guard = 0;
        while (!vga_480p) begin
            @(posedge clk);
            guard = guard + 1;
            if (guard > 4_000_000) begin
                $display("FAIL: 480p never activated");
                $fatal;
            end
        end
    end
    $display("480p active at t=%0t", $time);
    ...
    if (f1_toggles > 2)     begin $display("FAIL: F1 toggling in 480p"); $fatal; end
    if (vga_interlaced)     begin $display("FAIL: interlaced flag in 480p"); $fatal; end
